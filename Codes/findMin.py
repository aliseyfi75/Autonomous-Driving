...
def findMin_SGD(funObj, w, epochs, X, y, batch_size=500, alpha = 0.001, verbose=0):
    ...
    for epoch in range(1,epochs):
        ...
        # Evaluate the initial function value and gradient
        f, g = funObj(w, X[0:batch_size - 1, :], y[0:batch_size - 1, :])

        for iter in range(np.int(X.shape[0]/batch_size)):

            w_new = w - alpha * g
            f_new, g_new = funObj(w_new, X[iter*batch_size:(iter+1)*batch_size-1,:], y[iter*batch_size:(iter+1)*batch_size-1,:])

            # Print progress
            if verbose > 0:
                print("%d - loss: %.3f" % (iter, f_new))
